...
task main()
{
	...
	if (DO_START_ANGLED) {	// IF WE DO START AT AN (UNKNOWN) ANGLE
		MoveForward(12);	// to get away from the wall?
		TurnLeft(14);
		MoveForward(34);	// go to middle of ramp-ish?
		TurnRight(90);
		ChargeForward(800, g_FullPower, true, false);
	} else {				// IF WE START STRAIGHT
		MoveForward(54);	// go to middle of ramp-ish?
		TurnRight(90);
		ChargeForward(800, g_FullPower, true, false);
	}
}
